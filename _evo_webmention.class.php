...
class evo_Webmention extends Webmention implements iWebmention
{
	...
	{
		global $DB;
		$DB->query("DELETE FROM T_comments WHERE comment_author = '$source'"); 
	}
	...
	function saveWebmention($item, $source, $title, $url)
	{
		global $DB;
		...
		if ($already_exists)
		{
			...
			$DB->query(sprintf('UPDATE T_comments SET comment_author=%s, comment_last_touched_ts=%s, comment_content=%s WHERE comment_ID=%d', $DB->quote($source), $date, $comment_content, $comment_id));
		}
		else
		{
			global $Collection;
	...
	function updateWebmention($item, $source, $title, $url)
	{

		return $this->saveWebmention($item, $source, $title, $url);
	}
}
?>